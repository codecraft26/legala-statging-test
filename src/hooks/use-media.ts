...
export function useDownloadFile() {
  return useMutation<void, Error, { filePath: string }>({
    mutationFn: async ({ filePath }) => {
      ...
      const signedUrlResponse = await Api.post<SignedUrlResponse>("/media/signed/url", {
        file: filePath,
      });
...
export function useFetchFileContent() {
  return useMutation<Blob, Error, { filePath: string }>({
    mutationFn: async ({ filePath }) => {
      ...
      try {
        const response = await fetch(`/api/media/proxy?file=${encodeURIComponent(filePath)}`, {
          method: 'GET',
          headers: {
      ...
      throw new Error('All fetch attempts failed. The S3 bucket may not be configured for CORS, or the signed URL may be invalid.');
    },
    onError: (error) => {
