...
type Props = {
  workspaceId?: string;
  onLoadDraftContent?: (data: { id: string; name?: string; content?: string }) => void;
  ...
  const handleImportDraft = async () => {
    ...
    try {
      ...
      if (data && onLoadDraftContent) {
        onLoadDraftContent({ id: data.id, name: data.name, content: data.content || "" });
      }
    } catch (e) {
  ...
  const handleCreateNewDraft = () => {
    try {
      ...
      if (onLoadDraftContent) {
        onLoadDraftContent({ id: "", name: "New Draft", content: "" });
      }
